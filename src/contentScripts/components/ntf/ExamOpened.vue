...
<template>
  <CommonBody icon="mdi-note-edit" :time-stamp="props.data.timestamp">
    ...
    <template #content>
      {{ $t("ntf.exam_opened.0") }} <a class="ntfc-link" @click="openCourse(payload.course_id)">{{ payload.course_name }}</a> {{ $t("ntf.exam_opened.1") }} <a class="ntfc-link" @click="openActivity(payload.course_id, payload.exam_id)">{{ payload.exam_title }}</a> {{ $t("ntf.exam_opened.2", {
