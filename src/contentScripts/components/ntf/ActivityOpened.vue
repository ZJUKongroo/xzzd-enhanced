...
<template>
  ...
  <div class="ntfc-title">
    {{ $t("ntf.activity_opened.0") }}<a class="ntfc-link" @click="openCourse(payload.course_id)">{{ payload.course_name }}</a>{{ $t("ntf.activity_opened.1") }}<a class="ntfc-link" @click="openActivity(payload.course_id, payload.activity_id)">{{ payload.activity_title }}</a>{{ $t("ntf.activity_opened.2") }}
  </div>
